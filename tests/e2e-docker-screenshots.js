...
const CHROME_PATH = '/home/pptruser/.cache/puppeteer/chrome/linux-146.0.7680.76/chrome-linux64/chrome';
...
(async () => {
  ...
  try {
    ...
    await page.screenshot({ path: `${SCREENSHOT_DIR}/03-plugins.png`, fullPage: true });
    ...
    const chatInfo = await page.evaluate(() => {
      ...
      return {
        ...
        rootInnerHTML: root ? root.innerHTML.substring(0, 300) : 'NOT FOUND',
    ...
    if (textarea) {
      ...
      const responseInfo = await page.evaluate(() => {
        const root = document.getElementById('ai-agent-root');
        return {
          innerHTML: root ? root.innerHTML.substring(0, 1000) : 'NOT FOUND',
          textContent: root ? root.textContent.substring(0, 500) : 'NOT FOUND'
        };
      });
      console.log('Response info:', JSON.stringify(responseInfo).substring(0, 500));
    } else {
    ...
    const apiResult = await page.evaluate(async () => {
      ...
      } catch (e) { results.providersError = e.message; }

      try {
        const resp = await fetch('/?rest_route=/ai-agent/v1/settings', {
          headers: { 'X-WP-Nonce': window.wpApiSettings?.nonce || '' }
        });
    ...
    console.log('Providers:', JSON.stringify(apiResult.providers || apiResult.providersError).substring(0, 500));
    console.log('Settings provider:', apiResult.settings?.default_provider);
    ...
    console.log(`PHP errors: ${phpErrors.length > 0 ? JSON.stringify(phpErrors) : 'None found'}`);
