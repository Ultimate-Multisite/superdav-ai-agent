 ...
 */
export default function ModelSelector( { providers, selectedModels, onChange, disabled } ) {
				...
				if ( provider.models ) {
					provider.models.forEach( ( model ) => {
						...
						const exists = models.some(
							( m ) => m.provider_id === provider.id && m.model_id === model.id
						);
						if ( ! exists ) {
							models.push( {
	...
	const isSelected = ( model ) => {
		return selectedModels.some(
			( m ) => m.provider_id === model.provider_id && m.model_id === model.model_id
		);
	};
	...
	return (
		<div className="gratis-ai-model-selector">
			...
			<div className="gratis-ai-model-selector-actions" style={ { margin: '12px 0' } }>
				<Button
					variant="secondary"
					onClick={ selectAll }
					disabled={ disabled }
			...
			{ Object.entries( groupedModels ).map( ( [ providerName, models ] ) => (
				<div key={ providerName } className="gratis-ai-model-provider">
					<h4>{ providerName }</h4>
					<div className="gratis-ai-model-list">
						{ models.map( ( model ) => (
							<div key={ `${ model.provider_id }-${ model.model_id }` } className="gratis-ai-model-item">
								<CheckboxControl
			...
			{ filteredModels.length === 0 && (
				<p style={ { color: '#646970', textAlign: 'center', padding: '20px' } }>
					{ __( 'No models found matching your search.', 'gratis-ai-agent' ) }
				</p>
			) }
		</div>
	);
