...
class BenchmarkControllerTest extends WP_UnitTestCase {
	...
	public function test_routes_are_registered(): void {
		...
		$expected = [
			...
			'/gratis-ai-agent/v1/benchmark/suites/(?P<slug>[a-z0-9\-_]+)',
			'/gratis-ai-agent/v1/benchmark/runs',
			'/gratis-ai-agent/v1/benchmark/runs/(?P<id>\d+)',
			'/gratis-ai-agent/v1/benchmark/runs/(?P<id>\d+)/run-next',
			'/gratis-ai-agent/v1/benchmark/compare',
		];
	...
	public function test_suites_requires_auth(): void {
		...
		$response = $this->dispatch( 'GET', '/gratis-ai-agent/v1/benchmark/suites' );
	...
	public function test_get_suite_not_found(): void {
		wp_set_current_user( $this->admin_id );
		$response = $this->dispatch( 'GET', '/gratis-ai-agent/v1/benchmark/suites/nonexistent-suite-xyz' );
	...
	public function test_get_suite_builtin(): void {
		...
		$list_response = $this->dispatch( 'GET', '/gratis-ai-agent/v1/benchmark/suites' );
		...
		$response = $this->dispatch( 'GET', "/gratis-ai-agent/v1/benchmark/suites/{$first_suite}" );
		$this->assertStatus( 200, $response );
	}
	...
	public function test_list_runs_returns_200(): void {
		wp_set_current_user( $this->admin_id );
		$response = $this->dispatch( 'GET', '/gratis-ai-agent/v1/benchmark/runs' );
		$this->assertStatus( 200, $response );
		$this->assertIsArray( $response->get_data() );
	}
	...
	public function test_get_run_not_found(): void {
		wp_set_current_user( $this->admin_id );
		$response = $this->dispatch( 'GET', '/gratis-ai-agent/v1/benchmark/runs/999999' );
		$this->assertStatus( 404, $response );
	}
	...
	public function test_get_run_requires_auth(): void {
		wp_set_current_user( 0 );
		$response = $this->dispatch( 'GET', '/gratis-ai-agent/v1/benchmark/runs/1' );
		$this->assertStatus( 401, $response );
	}
	...
	public function test_delete_run_requires_auth(): void {
		wp_set_current_user( 0 );
		$response = $this->dispatch( 'DELETE', '/gratis-ai-agent/v1/benchmark/runs/1' );
		$this->assertStatus( 401, $response );
	}
	...
	public function test_run_next_not_found(): void {
		wp_set_current_user( $this->admin_id );
		$response = $this->dispatch( 'POST', '/gratis-ai-agent/v1/benchmark/runs/999999/run-next', [] );
		$this->assertStatus( 404, $response );
	}
	...
	public function test_run_next_requires_auth(): void {
		wp_set_current_user( 0 );
		$response = $this->dispatch( 'POST', '/gratis-ai-agent/v1/benchmark/runs/1/run-next', [] );
		$this->assertStatus( 401, $response );
	}
	...
	public function test_compare_requires_at_least_two_runs(): void {
		wp_set_current_user( $this->admin_id );
		$response = $this->dispatch( 'POST', '/gratis-ai-agent/v1/benchmark/compare', [
			'run_ids' => [ 1 ],
		] );
		$this->assertStatus( 400, $response );
	}
