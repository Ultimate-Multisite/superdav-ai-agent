		...
		<div className="gratis-ai-benchmark-run-details">
			...
			<Card>
				...
				<CardBody>
					...
					<p>
						<strong>{ __( 'Test Suite:', 'gratis-ai-agent' ) }</strong>{ ' ' }
						{ run.test_suite }
					</p>
					<p>
					...
					{ run.completed_at && (
						<p>
							<strong>{ __( 'Completed:', 'gratis-ai-agent' ) }</strong>{ ' ' }
						...
						<table className="wp-list-table widefat fixed striped">
							<thead>
								<tr>
									...
									<th>{ __( 'Total', 'gratis-ai-agent' ) }</th>
									<th>{ __( 'Correct', 'gratis-ai-agent' ) }</th>
									<th>{ __( 'Accuracy', 'gratis-ai-agent' ) }</th>
								</tr>
							</thead>
							<tbody>
								{ Object.values( byModel ).map( ( model ) => (
									<tr key={ model.model_id }>
										...
										<td>{ model.provider_id || __( 'Default', 'gratis-ai-agent' ) }</td>
										<td>{ model.total }</td>
										<td>{ model.correct }</td>
										<td>
											{ Math.round( ( model.correct / model.total ) * 100 ) }%
										</td>
									</tr>
								) ) }
							</tbody>
			...
			{ Object.keys( byCategory ).length > 0 && (
				<Card style={ { marginTop: '20px' } }>
					<CardHeader>
						<h3>{ __( 'Results by Category', 'gratis-ai-agent' ) }</h3>
					</CardHeader>
					<CardBody>
						<table className="wp-list-table widefat fixed striped">
							<thead>
						...
						<table className="wp-list-table widefat fixed striped">
							<thead>
								<tr>
									...
									<th>{ __( 'Question ID', 'gratis-ai-agent' ) }</th>
									<th>{ __( 'Category', 'gratis-ai-agent' ) }</th>
									<th>{ __( 'Model', 'gratis-ai-agent' ) }</th>
									<th>{ __( 'Correct', 'gratis-ai-agent' ) }</th>
									<th>{ __( 'Answer', 'gratis-ai-agent' ) }</th>
									<th>{ __( 'Latency', 'gratis-ai-agent' ) }</th>
								</tr>
							</thead>
							<tbody>
								{ results.map( ( result, index ) => (
									<tr key={ index }>
										<td>
											{ result.is_correct ? (
												<Icon icon={ check } style={ { color: '#1a7f37' } } />
											) : (
												<Icon icon={ closeSmall } style={ { color: '#cf222e' } } />
											) }
										</td>
										...
										>
											{ result.model_answer.substring( 0, 50 ) }
