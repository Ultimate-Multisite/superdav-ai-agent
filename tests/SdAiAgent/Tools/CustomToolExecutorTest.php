...
class CustomToolExecutorTest extends WP_UnitTestCase {
	...
	public function test_execute_action_fires_hook(): void {
		...
		add_action(
			'sd_ai_agent_test_executor_hook',
			function () use ( &$fired ) {
				$fired = true;
			}
		);
		...
		$tool = [
			'type'   => CustomTools::TYPE_ACTION,
			'config' => [ 'hook_name' => 'sd_ai_agent_test_executor_hook' ],
		];
		...
		$this->assertSame( 'sd_ai_agent_test_executor_hook', $result['hook_name'] );
	}

	/**
	 * Test execute ACTION auto-prefixes hook name with sd_ai_agent_.
	 */
	public function test_execute_action_auto_prefixes_hook_name(): void {
		$fired_hook = null;

		add_action(
			'sd_ai_agent_my_custom_hook',
			function () use ( &$fired_hook ) {
				$fired_hook = 'sd_ai_agent_my_custom_hook';
			}
		);
		...
		$this->assertSame( 'sd_ai_agent_my_custom_hook', $fired_hook );
	}
	...
	public function test_execute_action_passes_input_as_first_arg(): void {
		...
		$tool = [
			'type'   => CustomTools::TYPE_ACTION,
			'config' => [ 'hook_name' => 'sd_ai_agent_test_args_hook' ],
		];
	...
	public function test_execute_action_uses_arg_defs(): void {
		...
		$tool = [
			'type'   => CustomTools::TYPE_ACTION,
			'config' => [
				'hook_name' => 'sd_ai_agent_test_argdefs_hook',
				'args'      => [
					'first'  => 'default_a',
					'second' => 'default_b',
				],
