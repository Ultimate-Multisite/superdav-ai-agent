...
function BenchmarkPageApp() {
	...
	const loadSuites = async () => {
		try {
			const data = await apiFetch( { path: '/gratis-ai-agent/v1/benchmark/suites' } );
			setSuites( data );
		} catch ( error ) {
			setNotice( {
				status: 'error',
				message: __( 'Failed to load benchmark suites.', 'gratis-ai-agent' ),
			} );
		}
	};
	...
	const loadProviders = async () => {
		try {
			const data = await apiFetch( { path: '/gratis-ai-agent/v1/providers' } );
	...
	const loadRuns = async () => {
		try {
			const data = await apiFetch( { path: '/gratis-ai-agent/v1/benchmark/runs' } );
			setRuns( data.runs || [] );
		} catch ( error ) {
			setNotice( {
				status: 'error',
				message: __( 'Failed to load benchmark runs.', 'gratis-ai-agent' ),
			} );
		}
	};
	...
	const handleCreateRun = async () => {
		...
		if ( selectedModels.length === 0 ) {
			setNotice( {
				status: 'error',
				message: __( 'Please select at least one model.', 'gratis-ai-agent' ),
			} );
			return;
		}
		...
		try {
			...
			setNotice( {
				status: 'success',
				message: __( 'Benchmark run created. Starting tests...', 'gratis-ai-agent' ),
			} );
			...
			setNotice( {
				status: 'error',
				message: error.message || __( 'Failed to create benchmark run.', 'gratis-ai-agent' ),
			} );
			setIsLoading( false );
		}
	};
		...
		const runNext = async () => {
			try {
				...
				if ( result.status === 'completed' ) {
					...
					setNotice( {
						status: 'success',
						message: __( 'Benchmark completed!', 'gratis-ai-agent' ),
					} );
					loadRuns();
					return;
				}
				...
				setNotice( {
					status: 'error',
					message: error.message || __( 'Benchmark failed.', 'gratis-ai-agent' ),
				} );
				loadRuns();
			}
		};
	...
	const handleViewRun = async ( runId ) => {
		setIsLoading( true );
		try {
			const run = await apiFetch( { path: `/gratis-ai-agent/v1/benchmark/runs/${ runId }` } );
			setSelectedRun( run );
	...
	const handleDeleteRun = async ( runId ) => {
		if ( ! window.confirm( __( 'Are you sure you want to delete this benchmark run?', 'gratis-ai-agent' ) ) ) {
			return;
		}
			...
			<Card>
				...
				<CardBody>
					{ isRunning && runProgress && (
						<div className="gratis-ai-benchmark-progress">
							<Notice status="info" isDismissible={ false }>
								{ __( 'Benchmark is running...', 'gratis-ai-agent' ) }
							</Notice>
							<ProgressBar
							...
							<p>
								...
								{ __( 'questions completed', 'gratis-ai-agent' ) }
							</p>
						</div>
					) }
					...
					<TextControl
						...
						value={ runName }
						onChange={ setRunName }
						placeholder={ __( 'e.g., Claude vs GPT-4 Comparison', 'gratis-ai-agent' ) }
						disabled={ isRunning }
					/>
					...
					<TextareaControl
						...
						placeholder={ __( 'Optional description of this benchmark run', 'gratis-ai-agent' ) }
						rows={ 3 }
						disabled={ isRunning }
					/>
