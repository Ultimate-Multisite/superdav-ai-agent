 ...
 *
 * @package GratisAiAgent\Tests\Services
 * @license GPL-2.0-or-later
 */

namespace GratisAiAgent\Tests\Services;
...
class ChangeRevertServiceTest extends WP_UnitTestCase {
	...
	public function test_apply_revert_option_restores_scalar_value(): void {
		update_option( 'gratis_test_blogname_revert', 'New Name' );

		$change = $this->make_change( [
			'object_type'  => 'option',
			'object_id'    => 0,
			'field_name'   => 'gratis_test_blogname_revert',
		...
		$this->assertTrue( $result );
		$this->assertSame( 'Old Name', get_option( 'gratis_test_blogname_revert' ) );

		delete_option( 'gratis_test_blogname_revert' );
	}
	...
	public function test_apply_revert_option_restores_array_value(): void {
		$option_name = 'gratis_test_array_option_revert';
	...
	/**
	 * apply_revert() delegates unknown types to the gratis_ai_agent_revert_change
	...
	public function test_apply_revert_unknown_type_respects_filter(): void {
		...
		add_filter( 'gratis_ai_agent_revert_change', $filter, 10, 2 );
		...
		remove_filter( 'gratis_ai_agent_revert_change', $filter );

		$this->assertTrue( $filter_called, 'The gratis_ai_agent_revert_change filter should have been called.' );
		$this->assertTrue( $result );
	}
}
