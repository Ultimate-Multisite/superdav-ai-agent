...
use SdAiAgent\Core\AbilityHooks;
use WP_UnitTestCase;
...
class AbilityHooksTest extends WP_UnitTestCase {
	...
	public function tearDown(): void {
		remove_all_filters( 'sd_ai_agent_ability_args' );
		remove_all_filters( 'sd_ai_agent_ability_blocked' );
		remove_all_filters( 'sd_ai_agent_ability_result' );
		remove_all_actions( 'sd_ai_agent_before_ability' );
		remove_all_actions( 'sd_ai_agent_after_ability' );
		remove_all_actions( 'sd_ai_agent_ability_error' );
		parent::tearDown();
	}
	...
	/**
	 * before() fires sd_ai_agent_before_ability with correct args.
	 */
	public function test_before_fires_action_with_correct_args(): void {
		...
		AbilityHooks::before( 'sd-ai-agent/memory-save', [ 'category' => 'general', 'content' => 'test' ], 'call_001' );

		$this->assertSame( 'sd-ai-agent/memory-save', $captured['name'] );
	...
	public function test_before_returns_original_args_when_no_filter(): void {
		...
		$result = AbilityHooks::before( 'sd-ai-agent/memory-save', $args, 'call_002' );

		$this->assertSame( $args, $result );
	}

	/**
	 * before() returns filtered args when sd_ai_agent_ability_args filter is registered.
	 */
	public function test_before_returns_filtered_args(): void {
		add_filter(
			...
			function ( ?array $args, string $name, string $call_id ): ?array {
				if ( 'sd-ai-agent/memory-save' === $name ) {
		...
		$result = AbilityHooks::before( 'sd-ai-agent/memory-save', $args, 'call_003' );
	...
	public function test_before_handles_null_args(): void {
		$result = AbilityHooks::before( 'sd-ai-agent/memory-list', null, 'call_004' );
	...
	public function test_is_blocked_returns_false_by_default(): void {
		$blocked = AbilityHooks::is_blocked( 'sd-ai-agent/memory-save', [], 'call_005' );
	...
	public function test_is_blocked_returns_true_when_filter_blocks(): void {
		add_filter(
			...
			function ( bool $blocked, string $name ): bool {
				return 'sd-ai-agent/memory-save' === $name ? true : $blocked;
			},
			10,
			2
		);

		$blocked = AbilityHooks::is_blocked( 'sd-ai-agent/memory-save', [], 'call_006' );
	...
	public function test_is_blocked_does_not_block_other_abilities(): void {
		...
		$blocked = AbilityHooks::is_blocked( 'sd-ai-agent/memory-list', [], 'call_007' );
	...
	public function test_is_blocked_passes_all_params_to_filter(): void {
		...
		AbilityHooks::is_blocked( 'sd-ai-agent/memory-delete', [ 'id' => 42 ], 'call_008' );

		$this->assertSame( 'sd-ai-agent/memory-delete', $captured['name'] );
	...
	/**
	 * after() fires sd_ai_agent_after_ability with correct args.
	 */
	public function test_after_fires_action_with_correct_args(): void {
		...
		AbilityHooks::after( 'sd-ai-agent/memory-save', [ 'category' => 'general', 'content' => 'test' ], $result, 'call_009' );
	...
	public function test_after_returns_original_result_when_no_filter(): void {
		...
		$returned = AbilityHooks::after( 'sd-ai-agent/memory-save', [], $result, 'call_010' );

		$this->assertSame( $result, $returned );
	}

	/**
	 * after() returns filtered result when sd_ai_agent_ability_result filter is registered.
	 */
	public function test_after_returns_filtered_result(): void {
		add_filter(
			'sd_ai_agent_ability_result',
			function ( mixed $result, string $name ): mixed {
				if ( 'sd-ai-agent/memory-save' === $name && is_array( $result ) ) {
		...
		$result   = [ 'success' => true ];
		$returned = AbilityHooks::after( 'sd-ai-agent/memory-save', [], $result, 'call_011' );

		$this->assertSame( 'injected', $returned['extra'] );
	}

	/**
	 * after() fires sd_ai_agent_ability_error when result is WP_Error.
	 */
	public function test_after_fires_error_action_on_wp_error(): void {
		...
		AbilityHooks::after( 'sd-ai-agent/memory-save', [], $error, 'call_012' );
	...
	/**
	 * after() does NOT fire sd_ai_agent_ability_error on success.
	 */
	public function test_after_does_not_fire_error_action_on_success(): void {
		...
		AbilityHooks::after( 'sd-ai-agent/memory-save', [], [ 'success' => true ], 'call_013' );

		$this->assertFalse( $error_fired );
	}

	/**
	 * after() fires sd_ai_agent_after_ability even when result is WP_Error.
	 */
	public function test_after_fires_after_action_even_on_error(): void {
		...
		$error = new \WP_Error( 'test_error', 'Something went wrong' );
		AbilityHooks::after( 'sd-ai-agent/memory-save', [], $error, 'call_014' );
	...
	public function test_after_passes_all_params_to_result_filter(): void {
		...
		$result = [ 'memories' => [] ];
		$args   = null;
		AbilityHooks::after( 'sd-ai-agent/memory-list', $args, $result, 'call_015' );

		$this->assertSame( $result, $captured['result'] );
		$this->assertSame( 'sd-ai-agent/memory-list', $captured['name'] );
