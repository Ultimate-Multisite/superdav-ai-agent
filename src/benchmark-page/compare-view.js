...
export default function CompareView( { comparison } ) {
	if ( ! comparison || ! comparison.summary ) {
		return (
			<Card>
				<CardBody>
					<p>{ __( 'No comparison data available.', 'gratis-ai-agent' ) }</p>
				</CardBody>
			</Card>
		);
	}

	const { summary, by_model, by_category } = comparison;
						...
						<table className="wp-list-table widefat fixed striped">
							<thead>
								<tr>
									...
									<th>{ __( 'Avg Latency', 'gratis-ai-agent' ) }</th>
									<th>{ __( 'Total Tokens', 'gratis-ai-agent' ) }</th>
								</tr>
							</thead>
							<tbody>
								{ summary.map( ( row ) => (
