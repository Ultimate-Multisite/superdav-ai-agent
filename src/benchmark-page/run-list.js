...
export default function RunList( { runs, onViewRun, onDeleteRun, isLoading } ) {
	...
	const getAccuracy = ( run ) => {
		if ( ! run.results || run.results.length === 0 ) {
			return 0;
		}
		const correct = run.results.filter( ( r ) => r.is_correct ).length;
		return Math.round( ( correct / run.results.length ) * 100 );
	};
			...
			<Card>
				<CardBody>
					<div className="gratis-ai-benchmark-empty">
						<p>{ __( 'No benchmark runs yet.', 'gratis-ai-agent' ) }</p>
						<p>
							{ __( 'Create a new benchmark to get started.', 'gratis-ai-agent' ) }
						</p>
					</div>
				</CardBody>
			</Card>
						...
						<tbody>
							{ runs.map( ( run ) => (
								<tr key={ run.id }>
									...
									<td>
										<span
											className={ `gratis-ai-benchmark-status ${ getStatusClass( run.status ) }` }
										>
											{ run.status }
										</span>
									</td>
