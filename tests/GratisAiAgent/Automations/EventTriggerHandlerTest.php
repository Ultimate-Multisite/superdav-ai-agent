...
class EventTriggerHandlerTest extends WP_UnitTestCase {
	...
	public function set_up(): void {
		...
		$this->event_id = (int) EventAutomations::create( [
			...
			'hook_name'       => 'gratis_ai_agent_test_hook_' . uniqid(),
	...
	public function test_register_hooks_execute_event_run(): void {
		...
		$this->assertGreaterThan(
			0,
			has_action( 'gratis_ai_agent_run_event_automation', [ EventTriggerHandler::class, 'execute_event_run' ] ),
			'execute_event_run should be hooked to gratis_ai_agent_run_event_automation'
		);
	}
	...
	public function test_attach_hooks_skips_disabled_events(): void {
		$unique_hook = 'gratis_ai_agent_disabled_hook_' . uniqid();
	...
	public function test_execute_event_run_invalid_transient_returns_early(): void {
		$run_key = 'gratis_ai_agent_event_run_test_invalid';
	...
	public function test_execute_event_run_deletes_transient(): void {
		$run_key  = 'gratis_ai_agent_event_run_test_delete';
		$run_data = [
	...
	public function test_execute_event_run_missing_event_returns_early(): void {
		$run_key  = 'gratis_ai_agent_event_run_test_missing_event';
		$run_data = [
			'event_id'  => 999999,
			'prompt'    => 'Test prompt',
			'hook_name' => 'test_hook',
	 ...
	 * gratis_ai_agent_event_automation_complete action to capture the call.
	 */
	public function test_execute_event_run_fires_completion_action(): void {
		...
		add_action(
			'gratis_ai_agent_event_automation_complete',
		...
		$run_key  = 'gratis_ai_agent_event_run_test_complete_' . uniqid();
		$run_data = [
			'event_id'  => $this->event_id,
			'prompt'    => 'Test prompt',
			'hook_name' => 'gratis_ai_agent_test_hook',
		];
		...
		$this->assertTrue( $fired, 'gratis_ai_agent_event_automation_complete should fire' );
