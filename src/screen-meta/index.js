 ...
 * the current screen data injected by PHP (aiAgentScreenMeta.screenContext)
 ...
 * injected by PHP (aiAgentScreenMeta.screenContext).
 *
...
function ScreenMetaChat() {
	...
	useEffect( () => {
		const screenMeta = window.aiAgentScreenMeta;
	...
	return (
		<div className="ai-agent-screen-meta-wrap">
			<p className="ai-agent-screen-meta-intro">
				{ __(
 ...
 * opens the tab. We use a MutationObserver to detect when the mount
 * point becomes visible and then render the React app.
 *
...
if ( document.readyState === 'loading' ) {
	document.addEventListener( 'DOMContentLoaded', () => {
		const mountId =
			window.aiAgentScreenMeta?.mountId ?? 'ai-agent-screen-meta-root';
