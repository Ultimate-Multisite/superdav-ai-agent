...
readonly class ChangesLogRow {
	 ...
	 * @param string      $object_type  Object type ('post', 'option', 'post_meta', 'nav_menu', etc.).
	 ...
	 * @param bool        $revertable   Whether the change can be automatically undone.
	 ...
	 *
	 * Old rows without the revertable column default to true (pre-existing
	 * logged changes are assumed revertable unless proven otherwise).
	 *
	...
	public static function from_row( object $row ): self {
		return new self(
			...
			revertable:   isset( $row->revertable ) ? (bool) (int) $row->revertable : true,
